...
async function serveXtreamUi(res,pathname){
  let file='';if(pathname==='/xtream')file='admin.html';else if(pathname==='/xtream-assets/admin.css')file='admin.css';else if(pathname==='/xtream-assets/admin.js')file='admin.js';else return false;
  let body=await readFile(path.join(ROOT,'public',file),'utf8');
  if(file==='admin.html')body=body.replace('href="/admin.css"','href="/xtream-assets/admin.css"').replace('src="/admin.js"','src="/xtream-assets/admin.js"');
  if(file==='admin.js')body=body.replaceAll('/api/admin/','/xtream-api/');
...
function proxyTarget(requestUrl){const incoming=new URL(requestUrl||'/','http://blofy-gateway.local');let pathname=incoming.pathname,base=ACTIVATION_URL;if(pathname==='/releases-admin'||pathname.startsWith('/releases-admin/')){const suffix=pathname.slice('/releases-admin'.length);pathname=`/admin${suffix}`;base=RELEASE_URL}else if(RELEASE_PATH.test(pathname))base=RELEASE_URL;return new URL(pathname+incoming.search,`${base}/`)}
function proxy(req,res){
  ...
  const upstream=transport.request({protocol:target.protocol,hostname:target.hostname,port:target.port||undefined,method:req.method,path:`${target.pathname}${target.search}`,headers},upstreamRes=>{const responseHeaders={};for(const [name,value] of Object.entries(upstreamRes.headers)){if(value!=null&&!HOP_BY_HOP.has(name.toLowerCase())&&name.toLowerCase()!=='server')responseHeaders[name]=value}Object.assign(responseHeaders,securityHeaders());res.writeHead(upstreamRes.statusCode||502,responseHeaders);upstreamRes.pipe(res)});
  upstream.setTimeout(30_000,()=>upstream.destroy(new Error('upstream_timeout')));upstream.on('error',error=>{console.error(`gateway upstream error for ${target.href}:`,error.message);if(!res.headersSent)json(res,502,{ok:false,error:'bad_gateway'});else res.destroy(error)});req.on('aborted',()=>upstream.destroy());req.pipe(upstream);
}
