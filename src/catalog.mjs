...
export function stableNumericId(namespace, rawValue) {
  const digest = crypto.createHash('sha256').update(`${namespace}\0${rawValue}`).digest();
...
export function parseEpisodeTitle(title = '') {
  const text = String(title).trim();
  const patterns = [
    /^(.*?)[\s._-]+S(\d{1,2})E(\d{1,3})(?:\b|[\s._-])(.*)?$/i,
    /^(.*?)[\s._-]+(\d{1,2})x(\d{1,3})(?:\b|[\s._-])(.*)?$/i,
    /^(.*?)[\s._-]+Season\s*(\d{1,2})[\s._-]+Episode\s*(\d{1,3})(?:\b|[\s._-])(.*)?$/i,
    /^(.*?)[\s._-]+Episode\s*(\d{1,3})(?:\b|[\s._-])(.*)?$/i
  ];
