...
export async function syncIptvOrg() {
  ...
  const limit = envInt('IPTV_ORG_LIMIT', 5000, 50, 30000);
  ...
  for (const lineRaw of lines) {
    ...
    items.push({sourceItemId:attrs['tvg-id']||`${name}\0${line}`,kind:'live',title:name,description:'Publicly reachable stream indexed by IPTV-org. Verify channel rights for your intended use.',icon:attrs['tvg-logo']||'',category:attrs['group-title']||'Live TV',country:attrs['tvg-country']||'',language:attrs['tvg-language']||'',licenseName:'Rights review required',licenseUrl:'https://github.com/iptv-org/iptv',attribution:'IPTV-org directory / original broadcaster',stream:{resolver:'direct',url:line,extension:line.includes('.m3u8')?'m3u8':'ts'},rights:{mode:'directory-claim',redistributable:false,reviewRequired:true}});
