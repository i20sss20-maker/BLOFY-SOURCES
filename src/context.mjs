...
function normalizeAccount(account={}){
  return {
    ...
    maxConnections:Math.max(1,Math.min(20,Number(account.maxConnections)||1)),
    label:String(account.label||'').trim().slice(0,120),
    note:String(account.note||'').trim().slice(0,500),
    lastRenewedAt:account.lastRenewedAt||null
  };
}
function migrateState(raw={}){
  const accounts=[];
  if(Array.isArray(raw.accounts)) for(const a of raw.accounts){const n=normalizeAccount(a);if(n.username&&n.salt&&n.hash)accounts.push(n)}
  if(raw.account?.username&&!accounts.some(a=>a.username===raw.account.username)) accounts.push(normalizeAccount(raw.account));
...
function bootstrapAccount(){
  ...
  return {username:BOOTSTRAP_USERNAME,createdAt:BOOTSTRAP_CREATED_AT,expiresAt:null,enabled:true,maxConnections:1,label:'Bootstrap',note:'',bootstrap:true};
}
...
function publicAccount(a){
  if(!a)return null;
  const expired=Boolean(a.expiresAt&&new Date(a.expiresAt).getTime()<=Date.now());
  return {username:a.username,createdAt:a.createdAt,expiresAt:a.expiresAt||null,enabled:a.enabled!==false,expired,maxConnections:a.maxConnections||1,label:a.label||'',note:a.note||'',lastRenewedAt:a.lastRenewedAt||null,bootstrap:Boolean(a.bootstrap)};
}
export function listAccounts(){
  const rows=state.accounts.map(publicAccount);
  const bootstrap=bootstrapAccount();if(bootstrap)rows.push(publicAccount(bootstrap));
  return rows.sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));
}
export function getAccount(username=''){
  if(username){const found=state.accounts.find(a=>safeText(a.username,username));return found||((bootstrapAccount()&&safeText(BOOTSTRAP_USERNAME,username))?bootstrapAccount():null)}
  return state.accounts[0]||bootstrapAccount();
}
export function authenticateAccount(username,password){
  const a=state.accounts.find(row=>safeText(row.username,username));
  if(a){
    if(a.enabled===false)return null;
    if(a.expiresAt&&new Date(a.expiresAt).getTime()<=Date.now())return null;
    return safeHex(a.hash,hash(password,a.salt))?a:null;
  }
  const bootstrap=bootstrapAccount();
  return bootstrap&&safeText(username,BOOTSTRAP_USERNAME)&&safeText(password,BOOTSTRAP_PASSWORD)?bootstrap:null;
}
...
export async function createAccount({username='',password='',durationDays=30,maxConnections=1,label='',note=''}={}){
  const finalUsername=String(username||random('blofy',5).toLowerCase()).trim();
  const finalPassword=String(password||random('',12));
  if(!validateUsername(finalUsername))throw new Error('invalid_username');
  if(finalPassword.length<8||finalPassword.length>128)throw new Error('invalid_password');
  if(state.accounts.some(a=>a.username===finalUsername))throw new Error('username_exists');
  const days=Math.max(0,Math.min(3650,Number(durationDays)||0));
  const now=Date.now(),salt=crypto.randomBytes(16).toString('hex');
  ...
  state.accounts.push(account);await saveState();
  return {...publicAccount(account),password:finalPassword};
}
export async function resetAccount(){return createAccount({durationDays:365,label:'Generated account'})}
export async function renewAccount(username,days=30){
  const a=state.accounts.find(x=>x.username===String(username));if(!a)throw new Error('account_not_found');
  const add=Math.max(1,Math.min(3650,Number(days)||30))*86400000;
  const base=a.expiresAt&&new Date(a.expiresAt).getTime()>Date.now()?new Date(a.expiresAt).getTime():Date.now();
  a.expiresAt=new Date(base+add).toISOString();a.enabled=true;a.lastRenewedAt=new Date().toISOString();await saveState();return publicAccount(a);
}
...
export function createAdminSession(res,req){const payload=`${Date.now()+8*60*60_000}.${crypto.randomBytes(18).toString('base64url')}`;const token=`${payload}.${sign(payload)}`;const secure=baseUrl(req).startsWith('https://')?'; Secure':'';res.setHeader('set-cookie',`blofy_admin=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=28800${secure}`)}
