...
export async function adminApi(req,res,url){
  ...
  if(url.pathname==='/api/admin/status'&&req.method==='GET'){
    ...
    return json(res,200,{ok:true,baseUrl:baseUrl(req),stats:catalog.stats(),providers:providerDefinitions.map(p=>({id:p.id,name:p.name,kind:p.kind,enabled:p.enabled(),rights:p.rights,runtime:catalog.sources[p.id]||null})),account:account?{username:account.username,createdAt:account.createdAt}:null,...sync});
  }
  if(url.pathname==='/api/admin/sync'&&req.method==='POST')return json(res,200,await syncAll());
  if(url.pathname==='/api/admin/account/reset'&&req.method==='POST'){
    const creds=await resetAccount(),host=baseUrl(req);return json(res,200,{ok:true,host,...creds,m3u:`${host}/get.php?username=${encodeURIComponent(creds.username)}&password=${encodeURIComponent(creds.password)}&type=m3u_plus&output=ts`});
  }
  if(url.pathname==='/api/admin/catalog'&&req.method==='GET'){
    const kind=url.searchParams.get('kind')||'',q=String(url.searchParams.get('q')||'').trim().toLowerCase(),limit=Math.min(500,Math.max(1,Number(url.searchParams.get('limit')||100)));
    let rows=[...catalog.items.values()];if(kind&&['live','movie','series_episode'].includes(kind))rows=rows.filter(x=>x.kind===kind);if(q)rows=rows.filter(x=>`${x.title} ${x.category} ${x.source}`.toLowerCase().includes(q));
