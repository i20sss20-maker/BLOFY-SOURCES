...
test('Xtream HTTP surface and unified admin work end-to-end like a player',async()=>{
  ...
  try{
    ...
    r=await fetch(`${base}/admin-assets/admin.js`);const js=await r.text();assert.match(js,/\/admin-api\//);assert.match(js,/compat-accounts\/reset/);assert.match(js,/exportSubscribersBtn/);
