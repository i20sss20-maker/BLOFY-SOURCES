...
test('Xtream HTTP surface and unified admin work end-to-end like a player',async()=>{
  ...
  try{
    ...
    r=await fetch(`${authUrl}&action=get_live_streams&category_id=0`,{headers:{'accept-encoding':'gzip'}});assert.equal(r.headers.get('content-encoding'),'gzip');assert.equal(r.headers.get('content-length'),null);j=await r.json();assert.equal(j.length,1);assert.equal(j[0].stream_id,101);assert.equal(j[0].epg_channel_id,'ArabicTest.sa');
    ...
    r=await fetch(`${base}/get.php?username=testuser&password=TestPass-123&type=m3u_plus&output=ts`,{headers:{'accept-encoding':'gzip'}});assert.equal(r.status,200);assert.equal(r.headers.get('content-encoding'),'gzip');assert.equal(r.headers.get('content-length'),null);const m3u=await r.text();assert.match(m3u,/^#EXTM3U/);assert.match(m3u,/\/live\/testuser\/TestPass-123\/101\.ts/);assert.match(m3u,/\/movie\/testuser\/TestPass-123\/102\.mp4/);assert.match(m3u,/\/series\/testuser\/TestPass-123\/103\.mp4/);
